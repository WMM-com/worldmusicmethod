...
DROP FUNCTION IF EXISTS public.bulk_import_student_tags(jsonb);
...
BEGIN
  ...
  LOOP
    ...
    SELECT id INTO v_profile_id FROM profiles WHERE LOWER(email) = student_email LIMIT 1;
    ...
    -- Update profile tags
    UPDATE profiles SET tags = student_tags WHERE id = v_profile_id;
    ...
    LOOP
      ...
      IF tag_to_course ? tag THEN
        v_course_id := (tag_to_course->>tag)::UUID;
        
        -- Upsert enrollment
        INSERT INTO course_enrollments (user_id, course_id, enrollment_type, is_active)
        VALUES (v_profile_id, v_course_id, 'purchase', true)
    ...
    IF is_member THEN
      ...
      LOOP
        INSERT INTO course_enrollments (user_id, course_id, enrollment_type, is_active)
        VALUES (v_profile_id, c, 'subscription', true)
        ON CONFLICT (user_id, course_id) DO NOTHING;
      END LOOP;
    END IF;
  END LOOP;
