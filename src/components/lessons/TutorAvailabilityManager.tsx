...
import { AvailabilityTemplateManager } from './AvailabilityTemplateManager';
...
export function TutorAvailabilityManager() {
  ...
  const handleAdd = async () => {
    ...
    if (!isRecurring && !specificDate) { toast.error('Please select a date for the one-off slot'); return; }
                  ...
                  <div className="space-y-1.5">
                    {recurringSlots.map(slot => (
                      <div key={slot.id} className="flex items-center justify-between p-2.5 bg-muted rounded-md">
                        <div className="flex items-center gap-2">
                          <Badge variant="secondary" className="text-xs">{DAYS_OF_WEEK[slot.day_of_week!]}</Badge>
                          <span className="text-sm">{slot.start_time?.slice(0, 5)} – {slot.end_time?.slice(0, 5)}</span>
                          <span className="text-xs text-muted-foreground">({slot.timezone.replace(/_/g, ' ')})</span>
                        </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1.5">
                    {oneOffSlots.map(slot => (
                      <div key={slot.id} className="flex items-center justify-between p-2.5 bg-muted rounded-md">
                        <div className="flex items-center gap-2">
                          <Badge variant="outline" className="text-xs">{slot.specific_date}</Badge>
                          <span className="text-sm">{slot.start_time?.slice(0, 5)} – {slot.end_time?.slice(0, 5)}</span>
                          <span className="text-xs text-muted-foreground">({slot.timezone.replace(/_/g, ' ')})</span>
                        </div>
                        <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => handleDelete(slot.id)}>
